...
def create_button_bundle(
    screen, bundle_x, bundle_y, button_width, button_height, button_gap, bundle_color
):
    button_rectangles = []
    ...
    for index, label in enumerate(["True", "False"]):
        ...
        button_y = bundle_y
        pygame.draw.rect(
            screen, bundle_color, (button_x, button_y, button_width, button_height)
        )
        text = font.render(label, True, (255, 255, 255))
...
def create_button_bundles(
    ...
    for bundle_color in bundle_colors:
        button_rectangles = create_button_bundle(
            screen,
            bundle_x,
            bundle_y,
            button_width,
...
def create_left_side_button_bundle(screen):
    ...
    create_button_bundles(
        screen, button_width, button_height, button_gap, bundle_gap, bundle_colors[:2]
    )
    create_button_bundles(
        screen,
        button_width,
        button_height,
        button_gap,
        bundle_gap + (button_height + button_gap) * 2,
        bundle_colors[2:],
    )
