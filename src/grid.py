...
def print_values():
    ...
    for row in range(2):
        for col in range(3):
            ...
            checkbox_value = menu.get_widget(f'selector_{row}_{col}_checkbox').get_value()[0]
            print(f"Row {row + 1}, Column {col + 1}: Selector - {selector_value}, Checkbox - {checkbox_value}")

# Add cells to the menu
for row in range(2):
    for col in range(3):
        # Add selector box (with a single option to simulate checkbox behavior)
        selector = menu.add.selector(f'Selector {row + 1},{col + 1}:', [('1', 1)], default=0, onchange=None,
                                     selector_id=f'selector_{row}_{col}')

        # Add checkbox (simulated with a single option selector)
        checkbox = menu.add.selector(f'Checkbox {row + 1},{col + 1}', [('False', False), ('True', True)], default=0,
